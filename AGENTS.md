...
Use focused smoke tests for the changed surface. Run `dotnet run --project Tools\ModernUiTests\ModernUiTests.csproj` for subtitle pipeline checks. Build or run `Tools\SubtitleTests\SubtitleTests.vcxproj` from Visual Studio for native subtitle queue/pipeline coverage. For UI changes, run `npm run build` in `Examples\WhisperDesktop.Web` before packaging.
